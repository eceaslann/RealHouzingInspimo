...
namespace RealHouzing.Consume.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var response = await client.GetAsync("https://localhost:7033/api/Product");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync("https://localhost:7033/api/Category");
        ...
        public async Task<IActionResult> AddProduct(AddProductViewModel p)
        {
            ...
            var response = await client.PostAsync("https://localhost:7033/api/Product", content);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/Product/{id}");
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> UpdateProduct(int id)
        {
            ...
            var response2 = await client2.GetAsync("https://localhost:7033/api/Product");
            ...
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/Product/{id}");
            if (response.IsSuccessStatusCode)
            {
                var jsonData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateProduct(UpdateProductViewModel p)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(p);
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var response = await client.PutAsync("https://localhost:7033/api/Product/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
