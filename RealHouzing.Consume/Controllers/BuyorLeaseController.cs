...
namespace RealHouzing.Consume.Controllers
{
    ...
    public class BuyorLeaseController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7033/api/BuyLease");
        ...
        public async Task<IActionResult> AddBuyLease(AddBuyLeaseViewModel p)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7033/api/BuyLease", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/BuyLease/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/BuyLease/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateBuyLease(UpdateBuyLeaseViewModel p)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(p);
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var response = await client.PutAsync("https://localhost:7033/api/BuyLease/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
