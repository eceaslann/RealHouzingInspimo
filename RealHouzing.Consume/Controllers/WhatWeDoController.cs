...
namespace RealHouzing.Consume.Controllers
{
    public class WhatWeDoController : Controller
    {
        ...
        public async Task<IActionResult> AddWhatWeDo(AddWhatwedoViewModel p)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7033/api/WhatWeDo", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/WhatWeDo/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/WhatWeDo/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateWhatWeDo(UpdateWhatwedoViewModel p)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(p);
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var response = await client.PutAsync("https://localhost:7033/api/WhatWeDo/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
