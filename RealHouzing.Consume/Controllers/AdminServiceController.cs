...
namespace RealHouzing.Consume.Controllers
{
    public class AdminServiceController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var response = await client.GetAsync("https://localhost:7033/api/Service");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/Service/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> AddService(AddServiceViewModel p)
        {
            ...
            var response = await client.PostAsync("https://localhost:7033/api/Service", stringContent);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/Service/{id}");
            if (response.IsSuccessStatusCode)
            {
                var jsonData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceViewModel p)
        {
            ...
            var response = await client.PutAsync("https://localhost:7033/api/Service/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
