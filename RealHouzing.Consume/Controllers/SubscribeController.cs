...
namespace RealHouzing.Consume.Controllers
{
    ...
    public class SubscribeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var response = await client.GetAsync("https://localhost:7033/api/Subscribe");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/Subscribe/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/Subscribe/{id}");
            if (response.IsSuccessStatusCode)
            {
                var jsonData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateSubscribe(UpdateSubscribeViewModel p)
        {
            ...
            var response = await client.PutAsync("https://localhost:7033/api/Subscribe/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
