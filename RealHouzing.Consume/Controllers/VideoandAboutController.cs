...
namespace RealHouzing.Consume.Controllers
{
    ...
    public class VideoandAboutController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7033/api/VideoandAbout");
        ...
        public async Task<IActionResult> AddVideoandAbout(AddVideoandAboutViewModel p)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7033/api/VideoandAbout", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/VideoandAbout/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/VideoandAbout/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateVideoandAbout(UpdateVideoandAboutViewModel p)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(p);
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var response = await client.PutAsync("https://localhost:7033/api/VideoandAbout/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
