...
namespace RealHouzing.Consume.Controllers
{
    ...
    public class TeamController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var response = await client.GetAsync("https://localhost:7033/api/Team");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/Team/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> AddTeam(AddTeamViewModel p)
        {
            ...
            var response = await client.PostAsync("https://localhost:7033/api/Team", stringContent);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/Team/{id}");
            if (response.IsSuccessStatusCode)
            {
                var jsonData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateTeam(UpdateTeamViewModel p)
        {
            ...
            var response = await client.PutAsync("https://localhost:7033/api/Team/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
