...
namespace RealHouzing.Consume.Controllers
{
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> AddCategory(AddCategoryViewModel addCategoryViewModel)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7033/api/Category", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/Category/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/Category/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateCategory(UpdateCategoryViewModel p)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var response = await client.PutAsync("https://localhost:7033/api/Category/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
