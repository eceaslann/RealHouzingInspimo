...
namespace RealHouzing.Consume.Controllers
{
    ...
    public class AdminAboutController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7033/api/About");
        ...
        public async Task<IActionResult> AddAbout(AddAboutViewModel p)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7033/api/About", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:7033/api/About/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:7033/api/About/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateAbout(UpdateAboutViewModel p)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(p);
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var response = await client.PutAsync("https://localhost:7033/api/About/", content);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
